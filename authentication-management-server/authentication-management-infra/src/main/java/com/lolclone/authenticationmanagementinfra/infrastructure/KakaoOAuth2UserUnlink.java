...
public class KakaoOAuth2UserUnlink implements OAuth2UserUnlink {
    ...
    private static final String KAKAO_UNLINK_REQUEST_BODY = "target_id_type=user_id&target_id=%s";

    @Override
    public void unlink(UUID memberId, String accessToken) {
        ProviderProperties kakao = oauth2Properties.getProviderProperties(PROVIDER_NAME);
        String unlinkUrl = kakao.getBaseUrl() + kakao.getUnlink();
        HttpEntity<String> httpEntity = createHttpEntity(memberId, accessToken);
    ...
    private HttpEntity<String> createHttpEntity(UUID memberId, String accessToken) {
        HttpHeaders headers = new HttpHeaders();
        headers.setBearerAuth(accessToken);
        headers.setContentType(MediaType.APPLICATION_FORM_URLENCODED);
        String body = String.format(KAKAO_UNLINK_REQUEST_BODY, memberId);
